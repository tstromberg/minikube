...
var completionCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if args[0] != "bash" && args[0] != "zsh" && args[0] != "fish" {
			...
			if err != nil {
				exit.WithError("COMPLETION", "bash completion failed", err)
			}
		} else if args[0] == "zsh" {
			err := GenerateZshCompletion(os.Stdout, cmd.Parent())
			if err != nil {
				exit.WithError("COMPLETION", "zsh completion failed", err)
			}
		} else {
			err := GenerateFishCompletion(os.Stdout, cmd.Parent())
			if err != nil {
				exit.WithError("COMPLETION", "fish completion failed", err)
			}
		}
