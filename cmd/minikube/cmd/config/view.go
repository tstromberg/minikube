...
var configViewCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		err := View()
		if err != nil {
			exit.Error(reason.MkConfigView, "config view failed", err)
		}
	},
}
...
func View() error {
	...
	for k, v := range cfg {
		...
		if err != nil {
			exit.Error(reason.MkViewTmpl, "Error creating view template", err)
		}
		viewTmplt := ViewTemplate{k, v}
		err = tmpl.Execute(os.Stdout, viewTmplt)
		if err != nil {
			exit.Error(reason.MkViewExec, "Error executing view template", err)
		}
	}
	return nil
}
