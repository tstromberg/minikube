...
func Running(name string) ClusterController {
	...
	if err != nil {
		exit.Error(reason.MkCommandRunner, "Unable to get command runner", err)
	}
...
func Healthy(name string) ClusterController {
	...
	if err != nil {
		...
		exitTip("delete", name, problem.ServiceError)
	}

	if as == state.Paused {
		out.T(out.Shrug, `The control plane for "{{.name}}" is paused!`, out.V{"name": name})
		exitTip("unpause", name, problem.ServiceConfig)
	}
